...
ARVORE_BINARIA* local = (ARVORE_BINARIA*) malloc(sizeof(ARVORE_BINARIA));

    if(vazia(arvore)) return 1;

    local=arvore;
   
    int ponta=arvore->item.chave, baixo;
    
    if(local->esq != NULL){
        local=arvore->esq;

        baixo = local-> item.chave;
        if(ponta!=baixo){
            if(eBinariaBusca(local)!=1){
                return 0;
            }

            else if(eBinariaBusca(local)==1){
                return 1;
            }
